...
createVersionHeader "-debug"
time emcc .libs/libharfbuzz.dylib hb-ucdn/.libs/ucdn.o --pre-js version-header.js --post-js post.js -o harfbuzz-unoptimized.js -O2 --closure 0 -s "ALLOW_MEMORY_GROWTH=1" -s "EXPORTED_FUNCTIONS=$EXPORTED_FUNCTIONS"

echo "Building debug version $VERSION, no typed arrays"
createVersionHeader "-untyped-debug"
time emcc .libs/libharfbuzz.dylib hb-ucdn/.libs/ucdn.o --pre-js version-header.js --post-js post.js -o harfbuzz-untyped-unoptimized.js -O2 --closure 0 -s "USE_TYPED_ARRAYS=0" -s "ALLOW_MEMORY_GROWTH=1" -s "EXPORTED_FUNCTIONS=$EXPORTED_FUNCTIONS"

echo "Building production version $VERSION with full optimization"
createVersionHeader ""
time emcc .libs/libharfbuzz.dylib hb-ucdn/.libs/ucdn.o --pre-js version-header.js --post-js post.js -o harfbuzz.js -O2 --closure 1 -s "ALLOW_MEMORY_GROWTH=1" -s "EXPORTED_FUNCTIONS=$EXPORTED_FUNCTIONS"

echo "Building production version $VERSION with full optimization, no typed arrays"
createVersionHeader "-untyped"
time emcc .libs/libharfbuzz.dylib hb-ucdn/.libs/ucdn.o --pre-js version-header.js --post-js post.js -o harfbuzz-untyped.js -s "USE_TYPED_ARRAYS=0" -O2 --closure 1 -s "ALLOW_MEMORY_GROWTH=1" -s "EXPORTED_FUNCTIONS=$EXPORTED_FUNCTIONS"
