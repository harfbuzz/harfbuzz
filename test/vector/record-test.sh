...
usage() {
  cat <<EOF
Usage: $0 [-o TESTS_FILE] [-n TEST_NAME] [-e EXPECTED_FILE] [--expected-dir DIR] [--subset-dir DIR] HB_VECTOR FONT_FILE [VECTOR_OPTIONS] [TEXT]

Subset font, verify visual equivalence with hb-view, and append vector test line:
...
if [ "x${fontfile:0:1}" = "x-" ]; then
  echo "Specify font file before vector options." >&2
  exit 1
fi
if [ ! -f "$fontfile" ]; then
...
if ! echo "$hb_vector" | grep -q 'vector'; then
  echo "Specify hb-vector as first positional argument: got '$hb_vector'." >&2
  exit 1
fi

if [[ "$hb_vector" == */* ]]; then
  hb_vector_path="$hb_vector"
else
  hb_vector_path="$(command -v "$hb_vector" 2>/dev/null || true)"
fi
if [ -z "${hb_vector_path:-}" ] || [ ! -x "$hb_vector_path" ]; then
  echo "hb-vector not found or not executable: '$hb_vector'." >&2
  exit 1
fi
hb_vector="$hb_vector_path"
tool_dir="$(cd "$(dirname "$hb_vector")" && pwd)"
...
for arg in "$@"; do
  if ! $have_text && [ "x${arg:0:1}" = "x-" ]; then
    ...
    options+=("$arg")
    if ! is_vector_only_option "$arg"; then
