...
install: all
	...
	@for %h in ($(HB_HDRS)) do @copy %h $(PREFIX)\include\harfbuzz
	@if exist $(HARFBUZZ_ICU_DLL_FILENAME).dll for %h in ($(HB_ICU_HDRS)) do @copy ..\..\src\%h $(PREFIX)\include\harfbuzz
	@if exist $(HARFBUZZ_GOBJECT_DLL_FILENAME).dll for %h in ($(HB_GOBJECT_HDRS)) do @copy ..\..\src\%h $(PREFIX)\include\harfbuzz
